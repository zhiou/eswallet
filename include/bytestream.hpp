...
public:
    ...
    bytestream& append(int t)
    {
        ...
        padding_zero(length() + N - b.length());
        return *this += b;
    }
  ...
  bytestream split(size_t start, size_t length) const {
    if (start + length > m_mem->size()) {
      throw std::out_of_range("bytestream split out of range");
    }
    return bytestream(
        ebyte_vec(m_mem->begin() + start, m_mem->begin() + start + length));
  }
...
public:
  ...
  bytestream(bytestream &&other) { m_mem = std::move(other.m_mem); }
  ...
  const ebyte &operator[](int i) const { return m_mem->at(i); }
...
public:
  ...
  bool operator!=(const bytestream &other) const { return !(*this == other); }
...
public:
  bytestream tail(size_t aNum) const {
    if (length() < aNum) {
      throw std::invalid_argument("bytestream out of bounds");
    }
...
public:
  // MARK: functional
  template <class T> T map(std::function<T(const bytestream &)> atransformer) {
