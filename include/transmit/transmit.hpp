...
public:
  ...
  ~transmit() { eventbus::instance().remove_handler<event>(this); }
...
public:
  ...
  bytestream send(const bytestream &apdu,
                  bool security = true) noexcept(false) {
    bytestream resp;
...
public:
  void
  handle(event &e) override final { // TODO: use state machine to manage states
    event next;

    switch (e.msg_type) {
    case event_msg::connected: {
      next.msg_type = Driver::instance().is_ble() ? event_msg::will_authencate
                                                  : event_msg::will_handshake;
      break;
    }
...
template <class Driver> class transmit<Driver> : public event_handler {
