...
typedef struct tagBlkLogItem 
{
	...
	u64t		u64PerformanceCounter; // 高精度计数器，LogViewer以此来排序
	u1			pu1Reserved[0x1400 - sizeof(LOG_TYPE) - 0x80 - sizeof(u64t)*2 - sizeof(u4)*3 - 0x0800 * 2 - 0x08];
	
	// 拷贝构造函数
	tagBlkLogItem (const tagBlkLogItem& other)
	{
		EsMemCopy(this, sizeof(tagBlkLogItem), &other, sizeof(tagBlkLogItem));
	}
	tagBlkLogItem ()
	{
...
public:
	...
	virtual void Log(LOG_TYPE enLogType, const char* szFile, u4 u4Line, const char* szLog) = 0;
