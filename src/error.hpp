...
namespace excelsecu {
    ...
    static cos_err_desp_map cos_err_desp = {
        ...
        {"6A88",    "引用的数据未找到"},
        {"6A89",    "标识符已存在"},
        {"6A8A",    "指定的应用已打开""管理指令Tag错误，仅用于内部测试"},
        {"6A8B",    "协商密钥错误，不支持的算法""指定的应用不存在"},
        {"6A8C",    "引用的会话密钥不存在"},
        {"6A8D",    "数据错误(密钥句柄与EncryptInit命令指定的不符)"},
        {"6A90",    "已有打开的应用，当前设备不支持同时打开多个应用"},
        {"6A91",    "指定的容器未找到""修改PIN失败，恢复原有值"},
        {"6A92",    "同名文件已存在"},
        {"6A93",    "文件不存在"},
        {"6A94",    "引用的容器未找到"},
        {"6A95",    "容器中没有对应的密钥对"},
        ...
        {"6A98",    "正确执行，但验证签名失败"},
        {"6A99",    "不支持的会话密钥算法标识"},
        {"6A9A",    "非对称加密失败""线路保护协商密钥SM2加密出错"},
        {"6A9B",    "非对称解密失败""SM2解密哈希值校验出错"},
        {"6A9C",    "没有做自主认证""蓝牙盾没有认证特征密钥"    },
        {"6A9D",    "不支持的摘要算法标识"},
        {"6A9E",    "还有更多数据可取"},
        {"6AA6",     "NFC设备正忙"},
        ...
        {"6F02",    "NFC卡盾二次拍卡的时候，没空处理接收的指令，只能回此码告诉上层卡盾没准备好"},
        {"6F03",    "等待按键时，蓝牙断开连接"},
        {"6F04",    "音频支持插入指令的二代签名中间状态码，当需要2次按键确认时，第一次按键后要返回的操作码"},
        {"6F05",    "自主认证失败"},
        {"6F06",    "默认密码不允许操作"},
        {"6FD0",    "电量低"},
        {"6FE0",     "RSA计算错误"},
    ...
    class cos_code {
    public:
        explicit cos_code(uint16_t code)
        :m_value(0x00)
        {
        ...
        explicit cos_code(const std::string & code)
        :m_value(0x00)
        {
        ...
        uint16_t code() const{ //TODO - process XX
            ...
            for (size_t i = 0; i < m_cos_code.length(); i += 2)
            {
                ...
                if (byte[0] == 'X') { byte[0] = '0';}
                if (byte[1] == 'X') { byte[1] = '0';}
    ...
    private:
        void parse(const std::string &new_code) {  // 拆分cos_code
            for (auto iter = std::begin(get_desp_map()); iter != std::end(get_desp_map()); ++iter)
            {
        ...
        {
            if (code1.length() != code2.length()) { return false; } // 长度不同
            
            if (code1 == code2) { return true; }                    // 完全匹配
    ...
    public:
        ...
        static cos_err_desp_map& get_desp_map() {
            return cos_err_desp;
        }
    };
    ...
    public:
        ...
        es_error(int code, int module_id = 0xE060)   // 用于COS返回的错误码, module_id 默认为COS的moduleID
        : code_(cos_code(code)), module_(module_id), msg_("")
        {
            if (cos_code::get_desp_map().find(code_.code_str()) != std::end(cos_code::get_desp_map()))
            {
                msg_ = cos_code::get_desp_map()[code_.code_str()];
            }
        }
        
        es_error(const std::string &code, int module_id = 0xE060)   // 用于COS返回的错误码, module_id 默认为COS的moduleID
        ...
        es_error(const cos_code &code, int module_id = 0xE060)   // 用于COS返回的错误码, module_id 默认为COS的moduleID
        ...
        es_error(const std::string &msg, int code, int module_id = 0xE060) // 用于未定义的错误码
