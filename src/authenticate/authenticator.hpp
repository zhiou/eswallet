...
namespace excelsecu {
    ...
    public:
        ...
        std::string compare_code() const {
            return m_compare_code;
        }
        ...
        bytestream make_host_name()
        {
            ...
            auto host_name = bytestream(config_hostname.c_str(), config_hostname.length());
            if (host_name.length() > 0x20) { // 防止截取后出现乱码
                index = 0x1F;
                while(index > 0) {
                    if ((host_name[index] & 0x80) == 0) break;
                    if ((host_name[index] & 0xC0) == 0xC0) break;
                    if ((host_name[index] & 0xC0) == 0) index--;
                }
    ...
    private:
        ...
        // if auth mode is v5 spp, it just prepare the apdu for next step, and return the comparecode.
        // if auth mode is pair, it need promote the input dialog for paircode, so prepare returns a empty string.
        // it throws exception when error happened
        void authenticate() noexcept(false)
        {
            ...
            if (auth_data && auth_data->length() > 0) {
                ...
                {
                    throw tsm_err("prepare authenticate failed, invalid response", ERROR_COMMON_INVALID_DATA);
                }
                ...
                // 0X000000 || devcert(132) || Enc(sm2pubkey, rand+spkey+sid+comparenumber)(136) || devsign(0x40)
                if (repo.length() < 0x04 + 0x84 + 0x88 + 0x40) {
                    throw tsm_err("first time prepare authenticate failed, invalid response", ERROR_COMMON_INVALID_DATA);
                }
                ...
                auth_data.reset(new bytestream(feature_plain.split(0x10, 0x18))); // SPKey + SID + compare_number
            ...
            auto skey = cipher::encrypt(cipher::type::tdes112, auth_data->split(0, 0x10), rand2_randinv);
            auto enc_random = cipher::encrypt(cipher::type::tdes112, skey, random.split(0x00, 0x08));
