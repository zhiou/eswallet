    ...
    public:
        ...
        void shake_hand() noexcept(false){
            ...
            if (repo.length() < 0x84 + 0x74 + 0x40) {
                throw tsm_err("first time prepare authenticate failed, invalid response", 0x6700);
            }
            ...
            if (auth_str.length() > 0) {
                auto session_key = cipher::decrypt(cipher::type::sm4, bytestream(auth_str).front(0x10), skey);
    ...
    public:
        ...
        bytestream decrypt(const bytestream &repo)
        {
            if (m_session_context)
            {
                ...
                auto plain = cipher::decrypt(cipher::type::sm4, session_key, repo.front(repo.length() - 0x02));
        ...
        bytestream encrypt(const bytestream &apdu)
        {
            if (m_session_context)
            {
                ...
                auto padding_len = should_padding ? (0x10 - (apdu.length() % 0x10)) : 0x00;
                
                auto apdu_padding = bytestream(apdu) + bytestream("80") + bytestream(padding_len - 1);
                
                auto apdu_cipher = cipher::encrypt(cipher::type::sm4, session_key, should_padding ? apdu_padding : apdu);
