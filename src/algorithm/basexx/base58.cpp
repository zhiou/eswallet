...
bool DecodeBase58(const char *psz, std::vector<unsigned char> &vch) {
  ...
  int size =
      (int)strlen(psz) * 733 / 1000 + 1; // log(58) / log(256), rounded up.
...
std::string EncodeBase58(const unsigned char *pbegin,
  ...
  int size =
      (int)(pend - pbegin) * 138 / 100 + 1; // log(256) / log(58), rounded up.
...
std::string EncodeBase58Check(const std::vector<unsigned char> &vchIn) {
  ...
  u1 u1HashValue1[0x20], u1HashValue2[0x20];
  EsHashData(ES_HASH_ALG_SHA256, vch.data(), (u4)vch.size(), u1HashValue1);
  EsHashData(ES_HASH_ALG_SHA256, u1HashValue1, (u4)sizeof(u1HashValue1),
             u1HashValue2);

  vch.insert(vch.end(), u1HashValue1, u1HashValue1 + 4);
...
bool DecodeBase58Check(const char *psz, std::vector<unsigned char> &vchRet) {
  ...
  u1 u1HashValue1[0x20], u1HashValue2[0x20];
  EsHashData(ES_HASH_ALG_SHA256, vchRet.data(), (u4)vchRet.size() - 4,
             u1HashValue1);
  EsHashData(ES_HASH_ALG_SHA256, u1HashValue1, (u4)sizeof(u1HashValue1),
             u1HashValue2);

  if (memcmp(u1HashValue2, &vchRet[vchRet.size() - 4], 4) != 0) {
    vchRet.clear();
    return false;
  }
