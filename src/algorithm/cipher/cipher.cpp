    ...
    namespace cipher {
        u4 get_alg_id(type t) {
            ...
            bytestream encrypt(type t, const bytestream &key, const bytestream &plain)
            {
                ...
                u4 ret = EsSymmEncEcb(get_alg_id(t), key.bytes(), plain.bytes(), (u4)plain.length(), cipher.get(), &cipher_len);
                if (ret)
                {
            ...
            bytestream decrypt(type t, const bytestream &key, const bytestream &cipher)
            {
                ...
                u4 ret = EsSymmDecEcb(get_alg_id(t), key.bytes(), cipher.bytes(), (u4)cipher.length(), plain.get(), &plain_len);
                if (ret)
                {
        ...
        bytestream mac(type t, const bytestream &key, const bytestream &plain)
        {
            ...
            u4 ret = EsSymmMac(ES_SYMM_ALG_SM4,key.bytes(), "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", plain.bytes(), (u4)plain.length(), mac.get(), &mac_len);
            if (ret)
            {
