...
std::pair<std::string, std::string> gen_keypair() {
  ...
  return std::pair<std::string, std::string>(
      prikey.hex_str(), pubkeyX.hex_str() + pubkeyY.hex_str());
}
...
bytestream encrypt(const bytestream &plain, const bytestream &pubkey) {
  ...
  u4 ret = EsSm2Enc(pubkey.bytes(), pubkey.bytes() + 0x20, plain.bytes(),
...
bytestream decrypt(const bytestream &cipher, const bytestream &prikey) {
  ...
  u4 ret = EsSm2Dec(prikey.bytes(), cipher.bytes(), (u4)cipher.length(),
                    plain.get(), &plain_len);
  if (ret) {
...
bytestream sign(const bytestream &digest, const bytestream &prikey) {
  ...
  u4 ret =
      EsSm2Sign(prikey.bytes(), digest.bytes(), signature, signature + 0x20);
  if (ret) {
...
bool verify(const bytestream &digest, const bytestream &sign,
  ...
  return EsSm2Verify(pubkey.bytes(), pubkey.bytes() + 0x20, digest.bytes(),
                     sign.bytes(), sign.bytes() + 0x20) == 0;
}
} // namespace sm2
} // namespace excelsecu
