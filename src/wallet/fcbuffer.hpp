...
namespace fcbuffer{
    ...
    uint64_t encode_name(const std::string &name)
    {
        ...
        auto charidx = [&](char ch) {
            ...
            if (idx == std::string::npos) { throw tsm_err("invalid base32 character", ERROR_COMMON_INVALID_PARAM); }
            return idx;
        };
        
        if (name.length() > 12) {
            throw tsm_err("invalid name, length greater than 12", ERROR_COMMON_INVALID_PARAM);
        }
