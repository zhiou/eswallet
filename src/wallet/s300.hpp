...
public:
    ...
    void select(wallet::applet app) {
        ...
        if (app_id == m_cur_app_id) { return; }
        
        try {
            auto apdu = bytestream("00A4040008B000000000" + app_id);
            auto resp = m_trans->send(apdu); // 如果返回值不为9000，或者发送失败将会抛出异常
            m_cur_app_id = app_id;
        } catch (const es_error& e) {
    ...
    std::string get_pubkey(coin coin_type, const std::string &path, bool showing = false)
    {
    ...
    std::string get_address(coin coin_type, const std::string &path, bool showing = false, bool storing = false)
    {
        uint8_t flag = 0;
    ...
    json get_derive_data(coin coin_type, const std::string& path)
    {
        if (coin_type != coin::btc) {
            ...
            throw tsm_err("coin type don't support to derive data", ERROR_COMMON_FUNCTION_NOT_SUPPORT);
        }
    ...
    json get_address(coin coin_type, std::string pubkey, std::string chain_code, std::string addr_type, size_t from_index, size_t to_index)
    {
        if (coin_type != coin::btc)
        {
            LOG_ERROR("coin type don't support to get address");
            throw tsm_err("coin type don't support to get address", ERROR_COMMON_FUNCTION_NOT_SUPPORT);
        }
        
        bool is_external = (addr_type == "external");
        json network = btc::get_addr_prefix(m_btc_net);
        return btc::derive_address(network["pubKeyHash"], pubkey, chain_code, is_external, from_index, to_index);
    }
    
    std::string get_account_name(coin coin_type, size_t account_index, std::string pm_data, bool showing = false, bool storing = false)
    {
        if (coin_type != coin::eos)
        {
            LOG_ERROR("coin type don't support to get account name");
            throw tsm_err("coin type don't support to get account name", ERROR_COMMON_FUNCTION_NOT_SUPPORT);
        }
    ...
    json get_permission(coin coin_type, uint32_t account_index)
    {
        ...
        while(true) {
            ...
            {
                permissions.emplace_back(resp.split(2 + i * pm_data_size, pm_data_size));
            }
            
            size_t remain_pm_size = resp[0];
            if (remain_pm_size == 0) { break; }
            offset += pm_size;
        }
    ...
    void add_token(coin coin_type, json token)
    {
        if (coin_type != coin::eth)
        {
            ...
            throw tsm_err("coin type don't support to add token", ERROR_COMMON_FUNCTION_NOT_SUPPORT);
        }
...
private:
    ...
    json pase_pmdata(coin coin_type, uint32_t account_index, const bytestream &pm) {
        uint8_t type = pm[0];
        ...
        { // type[1] actor[8] name[8] path[20]
            ...
            if (pm_account_index != account_index) { return json(); }
...
private:
    ...
    std::vector<json> get_applet_versions() {
        std::vector<std::string> app_names{"HDWallet", "Manager", "Backup", "BTC", "ETH", "EOS"};
        std::vector<applet> applet_types{
            applet::hdwallet, applet::manager,
