...
namespace excelsecu {
    ...
    struct message_post_t {
        ...
        bool operator == (const message_post_t& _rhs) const {return reg == _rhs.reg && seq == _rhs.seq;}
        bool operator != (const message_post_t& _rhs) const {return !operator==(_rhs);}
    ...
    struct message_title_t {
        ...
        template<typename T> message_title_t(const T& _title):title((uintptr_t)_title) {static_assert(sizeof(T) <= sizeof(uintptr_t), "");}
        
        bool operator ==(const message_title_t& _rhs) const { return title == _rhs.title; }
        bool operator !=(const message_title_t& _rhs) const { return !operator==(_rhs); }
        uintptr_t title;
    };
    
    struct message_t {
        message_t(): title(0), anr_timeout(0), create_time(0), execute_time(0) {}
        message_t(const message_title_t& _title, const linb::any& _body1, const linb::any& _body2, const std::string _name)
        :title(_title), body1(_body1), body2(_body2), anr_timeout(10*60*1000), msg_name(_name), create_time(utility::get_tick_count()),
        execute_time(0){}
        
        template<class F>
        message_t(const message_title_t& _title, const F& _func, const std::string& _name = "")
        : title(_title), body1(std::make_shared<async_invoke_function>()), body2(), anr_timeout(10*60*1000), msg_name(_name),
        ...
        std::string to_string() const {
            std::stringstream ss;
            ss << "msg_name:" << msg_name << ", create_time:" << create_time << ", execute_time:" << execute_time << std::endl;
            return ss.str();
        }
    ...
    struct message_timing {
        ...
        : type(timing_t::kPeriod)
        , after(_after)
        , period(_period)
        {}
        
        message_timing(int64_t _after)
        : type(timing_t::kAfter)
        , after(_after)
        , period(0)
        {}
        
        message_timing()
        : type(timing_t::kImmediately)
        , after(0)
        , period(0)
        {}
    ...
    typedef std::function<void (const message_post_t& _id, message_t& _message)> message_handler;
    ...
    inline const message_handler_t& post2handler(const message_post_t& _postid) { return _postid.reg; }
    ...
    message_handler_t def_async_invoke_handler(const message_queue_t& _msg_queue = current_thread_message_queue());
    ...
    message_handler_t install_message_handler(const message_handler_t& _handler,
                                              bool _recv_broadcast = false,
    ...
    message_post_t post_message(const message_handler_t& _handler_id, const message_t& _msg, const message_timing& _timing = kDefTiming);
    message_post_t singleton_message(bool _replace, const message_handler_t& _handlerid, const message_t& _msg, const message_timing& _timing = kDefTiming);
    message_post_t broadcast_message(const message_queue_t& _msg_queue_id, const message_t& _msg, const message_timing& _timing = kDefTiming);
    message_post_t faster_message(const message_handler_t& _handler_id, const message_t& _msg, const message_timing& _timing = kDefTiming);
    ...
    void cancel_message(const message_handler_t& _handler_id, const message_title_t& _title);
    ...
    template<class F>
    message_post_t async_invoke(const F& _func, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name = "default_name") {
        return post_message(_handlerid, message_t(0, _func, _msg_name));
    }
    
    template<class F>
    message_post_t async_invoke_after(int64_t _after, const F& _func, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name = "default_name")
    {
        return post_message(_handlerid, message_t(0, _func, _msg_name), message_timing(kAfter, _after, 0));
    }
    
    template<class F>
    message_post_t async_invoke_period(int64_t _after, int64_t _period, const F& _func, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name = "default_name")
    {
        return post_message(_handlerid, message_t(0, _func, _msg_name), message_timing(kPeriod, _after, _period));
    }
    
    template<class F>
    message_post_t async_invoke(const F& _func, const message_title_t& _title, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name = "default_name") {
        return post_message(_handlerid, message_t(_title, _func, _msg_name));
    }
    
    template<class F>
    message_post_t async_invoke_after(int64_t _after, const F& _func, const message_title_t& _title, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name = "default_name")
    {
        return post_message(_handlerid, message_t(_title, _func, _msg_name), message_timing(kAfter, _after, 0));
    }
    
    template<class F>
    message_post_t async_invoke_period(int64_t _after, int64_t _period, const F& _func, const message_title_t& _title, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name = "default_name")
    {
        return post_message(_handlerid, message_t(_title, _func, _msg_name), message_timing(kPeriod, _after, _period));
    }
    ...
    public:
        message_queue_creater(bool _is_create = false, const char* _msg_queue_name = nullptr);
        message_queue_creater(std::shared_ptr<runloop_cond> _breaker, bool _is_create = false, const char* _msg_queue_name = nullptr);
    ...
    private:
        struct AsyncResultWrapper {
            ...
            AsyncResultWrapper(R* _result): result_holder(nullptr), result_valid(false), result(_result) {}
    ...
    private:
        void Invoke(const R& _result) const {
            ...
            if (wrapper_->callbcak_function) wrapper_->callback_function(_result, true);
        }
    private:
    ...
    public:
        ...
        template<typename T, typename C>
        AsyncResult(const T& _func, const C& _callback, const void* _place_holder = nullptr)
        :wrapper_(new AsyncResultWrapper()) {
            static_assert(std::is_same<typename std::result_of<T()>::type, void>::value, "");
            wrapper_->invoke_function = _func;
            wrapper_->callback_function = _callback;
    ...
    template<typename R>
    R& wait_invoke(const AsyncResult<R>& _func, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name="default_name")
    {
    ...
    template<typename F>
    typename std::result_of<F()>::type wait_invoke(const F& _func, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name="default_name")
    {
        if (current_thread_message_queue() == handler2queue(_handlerid))
        {
            return _func();
    ...
    R wait_invoke(const F& _func, R _ret, long _timeout = -1, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name="default_name")
    {
        if (current_thread_message_queue() == handler2queue(_handlerid))
        {
            return _func();
            ...
            AsyncResult<R> result(_func);
            bool has_run = wait_message(async_invoke(result, _handlerid, _msg_name), _timeout);
            if (has_run) {
                return result.Result();
            } else {
                return _ret;
    ...
    template<typename R>
    message_post_t async_invoke(const AsyncResult<R>& _func, const message_handler_t& _handlerid = def_async_invoke_handler(), const std::string& _msg_name="default_name")
    {
        return post_message(_handlerid, message_t(0, _func, _msg_name));
    }
