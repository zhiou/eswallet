...
public:
  ...
  static thread_tid current_thread_id() { return pthread_self(); }
...
template <class T> class RunnableFunctor : public Runnable {
public:
...
template <class T> class RunnableFunctor<T *> : public Runnable {
public:
...
template <> class RunnableFunctor<Runnable> : public Runnable {
  RunnableFunctor();
};
...
template <class T> struct TransformImplement {
  static Runnable *transform(const T &t) { return new RunnableFunctor<T>(t); }
};

template <class T> inline Runnable *transform(const T &t) {
...
private:
  ...
  public:
    RunnableReference(Runnable *_target)
        : target(_target), count(0), tid(0), isjoined(false), isended(true),
          aftertime(LONG_MAX), periodictime(LONG_MAX),
          iscanceldelaystart(false), condtime(), splock(), isinthread(false),
          killsig(0) {
  ...
  public:
    ...
    SpinLock splock;
    bool
        isinthread; // 猥琐的东西，是为了解决线程还没有起来的时就发送信号出现crash的问题
    int killsig;
...
public:
  ...
  explicit Thread(const T &op, const char *_thread_name = NULL,
    ...
    if (_thread_name)
      strncpy(runable_ref_->thread_name, _thread_name,
  ...
  int start(bool *_newone = NULL) {
    ...
    int ret = pthread_create(reinterpret_cast<thread_tid *>(&runable_ref_->tid),
                             &attr_, start_routine, runable_ref_);
    assert(0 == ret);
  ...
  template <typename T> int start(const T &op, bool *_newone = NULL) {
    std::unique_lock<SpinLock> lock(runable_ref_->splock);
  ...
  int start_after(long after) {
    ...
    int ret = pthread_create(reinterpret_cast<thread_tid *>(&runable_ref_->tid),
                             &attr_, start_routine_after, runable_ref_);
    assert(0 == ret);
  ...
  int start_periodic(long after, long periodic) { // ms
    ...
    int ret = pthread_create(reinterpret_cast<thread_tid *>(&runable_ref_->tid),
                             &attr_, start_routine_periodic, runable_ref_);
    assert(0 == ret);
  ...
  int join() const {
    ...
    if (tid() == ThreadUtil::current_thread_id())
      return EDEADLK;
  ...
  int unsafe_exit() const {
    ...
    assert2(
        false,
        "In Android, use SIGUSR2(handler call pthread_exit) to pthread_cancel");
    return kill(SIGUSR2);
#else
  ...
  thread_tid tid() const { return runable_ref_->tid; }

  bool is_runing() const { return !runable_ref_->isended; }
  ...
  const char *thread_name() const { return runable_ref_->thread_name; }

private:
#ifdef ANDROID
  static void exit_handler(int _sig) { pthread_exit(0); }
#endif
  ...
  static void init(void *arg) {
    ...
    std::unique_lock<SpinLock> lock(
        (const_cast<RunnableReference *>(runableref))->splock);
    assert(runableref != 0);
    ...
    if (0 < strnlen((const char *)runableref->thread_name,
                    sizeof(runableref->thread_name))) {
#ifdef __APPLE__
      pthread_setname_np((const char *)runableref->thread_name);
#elif defined(ANDROID)
      pthread_setname_np(runableref->tid,
                         (const char *)runableref->thread_name);
#else
