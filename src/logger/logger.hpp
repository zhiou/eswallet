...
namespace excelsecu {
    ...
    public:
        Logger(const std::string &name, sink_shared_ptr single_sink): m_name(name), m_sink_ptr(std::move(single_sink)) {
        }
    ...
    public:
        void set_level(Priority priority) {
            m_prioryty = priority;
        }
        
        static Priority default_level() {
            return Priority::info;
        }
        
        Priority level() const {
            return m_prioryty;
        }
    private:
        bool should_log(Priority priority) {
            return true;
        }
